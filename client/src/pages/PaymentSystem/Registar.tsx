...
const RegPaymentRequest: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchLectures() {
      try {
        const response = await axios.get(`/lecture/reg/payment-request/${userId}`);
  ...
  const handleAccept = async (lecturerId: string) => {
    try {
      await axios.put(`/lecture/reg/payment-request/${lecturerId}`);
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <button
                          className={`text-blue-600 hover:text-blue-900 ml-2 p-2 pl-3 pr-3 ${
                            lecturer.paymentapproved.registrar ? 'bg-gray-400 text-gray-600 cursor-not-allowed' : 'bg-blue-400 text-white hover:bg-blue-500'
                          } rounded-xl m-1`}
                          onClick={() => handleComment(lecturer._id)}
                          disabled={lecturer.paymentapproved.registrar}
                        >
                          {lecturer.paymentapproved.registrar ? 'Comment' : 'Comment'}
                        </button>
                      </td>
                    </tr>
                  ))}
...
export default RegPaymentRequest;
