...
const ViceChancellorPaymentRequest: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchLectures() {
      try {
        const response = await axios.get(`/lecture/vice/payment-request/${userId}`);
  ...
  const handleAccept = async (lecturerId: string) => {
    try {
      await axios.put(`/lecture/vice/accept/payment-request/${lecturerId}`);
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <button
                          className={`text-blue-600 hover:text-blue-900 ml-2 p-2 pl-3 pr-3 ${
                            lecturer.paymentapproved.viceChancellor ? 'bg-gray-400 text-gray-600 cursor-not-allowed' : 'bg-blue-400 text-white hover:bg-blue-500'
                          } rounded-xl m-1`}
                          onClick={() => handleComment(lecturer._id)}
                          disabled={lecturer.paymentapproved.viceChancellor}
                        >
                          {lecturer.paymentapproved.viceChancellor ? 'Comment' : 'Comment'}
                        </button>
                      </td>
                    </tr>
                  ))}
...
export default  ViceChancellorPaymentRequest;
