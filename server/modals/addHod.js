...
const Hod = require('./Hod'); // Assuming Hod model is defined in Hod.js
const deans = require('./Dean'); 
const addHODData = async () => {
    try {
        const hodData = [
            {
      ...
      for (const hod of hodData) {
        ...
        const dean = await deans.findOne({ email: hod.deanEmail });
